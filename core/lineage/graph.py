...
def subset_codes(subset: pd.DataFrame) -> list[str]:
    """Возвращает очищенный список Code из подмножества диссертаций."""
    if "Code" not in subset.columns or subset.empty:
        return []
    return [c for c in subset["Code"].astype(str).str.strip().tolist() if c]


def subset_by_codes(df: pd.DataFrame, codes: list[str] | set[str]) -> pd.DataFrame:
    """Возвращает строки df по списку Code."""
    ...
    if not normalized:
        return df.iloc[0:0].copy()
    work = df.copy()
    code_series = work["Code"].astype(str).str.strip()
    return work[code_series.isin(normalized)]
