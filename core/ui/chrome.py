...
- Под каждым построенным деревом доступен блок количественных метрик. Значения относятся к текущему варианту дерева и учитывают выбранные фильтры.
- Внизу есть кнопка **«🔗 Поделиться»**, которая генерирует ссылку на текущий выбор ученых.
""",

    "dissertations": """
## Подвкладка «Поиск по формальным признакам»

На этой подвкладке доступен поиск диссертаций по формальным критериям. Код диссертации соответствует коду автореферата диссертации на сайте Национальной электронной библиотеки ([http://rusneb.ru](http://rusneb.ru)).
...
""",
    ...
    "profiles": """
## Подвкладка «Поиск по тематическим профилям»

На этой подвкладке реализован содержательный поиск диссертаций. Он основан не на совпадении слов в заголовке, а на анализе всего текста автореферата диссертационной работы. Поиск осуществляется с использованием иерархического классификатора, содержащего различные критерии, отражающие объект, процесс и результат диссертационного исследования.
