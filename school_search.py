...
def _person_key(name: str) -> Optional[Tuple[str, str, str]]:
    """
    Извлекает структурный ключ (фамилия, инициал_имени, инициал_отчества).
...
def _dedup_result_df(df: pd.DataFrame, metric_col: str) -> pd.DataFrame:
    """
    Удаляет дубликаты из результирующей таблицы поиска.
    ...
    Из группы дубликатов остаётся только строка с самым длинным именем
    руководителя (полное ФИО > инициалы). Нумерация # переписывается.
    ...
    def _row_key(row) -> tuple:
        ...
        return (person_part, str(row[metric_col]))
...
def get_all_roots(df: pd.DataFrame) -> List[str]:
    ...
    Финальная дедубликация по _person_key выполняется в _dedup_result_df
...
def search_by_members_in_year(
    ...
    for root in roots:
        ...
        if count == 0:
            continue
        rows.append(
            build_result_row(0, root, count, subset, metric)
        )
