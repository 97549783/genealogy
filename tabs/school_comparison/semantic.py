...
from core.semantic.section_vectors import (
    build_dissertation_section_vectors, composite_distance_matrix,
)
...
PER_SECTION_COLUMNS = [
    ...
    "Число диссертаций", "Полнота данных, %",
]
...
def gather_semantic_school_dataset(
    *, root: str, member_codes: Collection[str], section_index: pd.DataFrame,
    ...
    return SemanticSchoolDataset(root, vectors, included.reset_index(drop=True), coverage,
    ...
    *, datasets: Mapping[str, SemanticSchoolDataset], selection: SectionSelection,
    ...
    if len({label for label in labels}) < 2 or any(labels.count(label) < 2 for label in set(labels)):
        return SemanticSchoolComparisonResult(
            None, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(columns=PER_SECTION_COLUMNS),
    ...
    if distance_matrix is None:
        message = ("Попарный анализ превышает настроенный предел числа диссертаций. Сузьте состав школ или набор разделов."
                   if diagnostics.undefined_pair_count == 0
    ...
    *, datasets: Mapping[str, SemanticSchoolDataset], selection: SectionSelection,
    ...
    for section_key in selection.section_keys:
        eligible = {
            ...
            if sum(section_key in vectors for vectors in dataset.dissertation_vectors.values()) >= 2
        }
        if len(eligible) < 2:
            continue
        items, labels = [], []
        ...
        for label, dataset in enumerate(eligible.values()):
            section_items = [vectors[section_key] for vectors in dataset.dissertation_vectors.values() if section_key in vectors]
        ...
        if matrix is None or diagnostics.undefined_pair_count:
            continue
