...
def render_single_school_mode(
    ...
    blocks = get_available_block_columns(dataset)
    if classifier_labels:
        for block in blocks:
            block["label"] = classifier_labels.get(block["code"], block["label"])
    ...
    missing_count = sum(1 for group in __import__("tabs.articles.blocks", fromlist=["ARTICLE_ANALYSIS_BLOCK_GROUPS"]).ARTICLE_ANALYSIS_BLOCK_GROUPS.values() for item in group["codes"] if item["code"] not in dataset.columns)
    with st.expander("Диагностика классификатора", expanded=False):
        st.write(f"Недоступных выбранных кодов классификатора: {missing_count}")
