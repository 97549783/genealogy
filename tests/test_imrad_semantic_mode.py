...
from tabs.articles.imrad_section_labels import (
    format_article_label_ru,
    format_keywords_ru,
    section_label_ru,
)
...
def _seed_imrad_db(path: Path, matrix_rel: str) -> None:
    ...
    rows = [
        ...
        ("u2", "a1", "imrad_subblock", "RESULTS_OR_DEMONSTRATION", "own_results_or_findings", "zone", 0.8, 0, 0, "[]", "src"),
        ("u3", "a2", "imrad_block", "METHOD_OR_APPROACH", None, "zone", 0.4, 0, 0, "[]", "src"),
    ]
    for r in rows:
    ...
    conn.execute("INSERT INTO article_imrad_unit_texts VALUES ('t3','u1','a1','ru','compact_ru','русский текст 1','[\"рус1\"]')")
    conn.execute("INSERT INTO article_imrad_unit_texts VALUES ('t4','u2','a1','ru','canonical_ru','русский текст 2','[\"рус2\"]')")
    conn.execute("INSERT INTO article_imrad_unit_texts VALUES ('t5','u3','a2','en','compact_en','txt3','[]')")
...
def test_helpers_and_filters(monkeypatch, tmp_path: Path) -> None:
    ...
    ru = load_imrad_display_texts_ru(["u1", "u3"])
    assert ru[ru["unit_id"] == "u1"].iloc[0]["display_text_ru"] == "русский текст 1"
    assert ru[ru["unit_id"] == "u3"].empty
