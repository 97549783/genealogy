...
def test_metric_tables():
    ...
    assert base.loc[base["key"] == "genealogical_index", "Метрика"].iloc[0] == "Генеалогический индекс"
    assert base.loc[base["key"] == "genealogical_index", "Статус"].iloc[0] == "пока не рассчитывается"
    ...
    assert full.loc[full["key"] == "genealogical_index", "Значение"].iloc[0] is None
